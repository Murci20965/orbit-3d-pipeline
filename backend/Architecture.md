...
2. **Concurrent AI Processing:**  
   * **Tripo3D (Generation):** If an image is uploaded, the backend securely uploads it to Tripo's servers to receive an image_token, then initiates the v3.1 generation sequence.
   * **Groq (Contextualization):** Simultaneously, the input is routed to Groq. Uploaded images are Base64-encoded and passed to meta-llama/llama-4-scout-17b-16e-instruct to visually identify the object.
3. **The Optimization Layer (Headless Blender):** Raw AI meshes are downloaded to a secure temp/ volume. A subprocess triggers a Python (bpy) script inside a Headless Blender instance.
4. **The Presentation Layer:** Next.js dynamically imports the 3D Canvas (bypassing SSR) and renders the Draco-compressed .glb using Image-Based Lighting (IBL) and Contact Shadows.

## **3. Challenges & Engineering Solutions**

* **The WebGL SSR Conflict:** Next.js Server-Side Rendering (SSR) attempts to render components in a Node.js environment lacking the window object, which crashes Three.js. **Solution:** Implemented dynamic imports with `ssr: false`.
* **The OS-Level Blender Dependency:** Running Blender's bpy module directly on local machines causes severe library conflicts (libgl1, libxrender1). **Solution:** Containerized the entire backend using Docker.
* **GPU Resource Management:** Utilizing heavy cinematic shadow mapping in React Three Fiber caused WebGL Shader Errors (1282) on lower-end hardware. **Solution:** Transitioned to optimized IBL with contact shadows.
...
* **Decoupled Task Queues:** Separate the FastAPI router from the Blender optimization layer using Celery and Redis.
* **Global Caching:** Implement an AWS S3 + CloudFront CDN layer for asset delivery.
